...
public class DisplayPanel extends JPanel implements ActionListener //create display panel
{
    ...
    private int ButtonsWidth = 200; //button panel its 200 pixels wide // 버튼쪽 폭
    private final Color LB = new Color(0xAD,0xD8, 0xE6);  //Light Blue 		// 패널 배경 색
    private final Color DB = new Color(0x1E,0x90, 0xFF);  //dodger blue		// 게임 배경 색
    private final Color P = new Color(0x80,0, 0x80);  //purple blank number 
   
    public DisplayPanel()   //construct the sudoku display panel
    {       
        addMouseListener(new MouseAdapter() //we listen for mouse clicks on this panel		// 마우스의 클릭을 기다림
        {
            public void mousePressed(MouseEvent e) 
            {           	
                selectNumber(e.getX(),e.getY()); //마우스를 클릭할 경우 좌표 값을 가져옴        
        ...
        JPanel pb = new JPanel();   //create the button panel		//버튼 그리기
        ...
        SButton EYS = new SButton("Start Your Sudoku", "EYS");
        EYS.addActionListener(this);
        SButton HINT = new SButton("Hint", "HINT");
    ...
    */    this.add(pb,BorderLayout.WEST); //add the push button panel to the display panel       
    ...
    private void selectNumber(int x, int y)   //called method on mouse click // 마우스 클릭을 해서 좌표 값을 가져오면 숫자를 선택함
    {
    	...
    	for(count = 0; count < 9; count++) //check to find position
    	{
    		if(x > NumberPosition[count])
    			Xposition = count;  //it must be this(or the next) // X축 값 포지션
    	...
    	for(count = 0; count < 9; count++) //check to find position
    	{
    		if(y > NumberPosition[count])
    			Yposition = count;  //it must be this(or the next) // Y 위치
    	}//end of get y position
    	byte position = (byte) (Xposition + Yposition*9); // 0 ~ 80까지 입력 포지션
	...
	public void actionPerformed(ActionEvent e)  // ** 버튼을 클릭할 경우 이벤트를 발생시킨다. **
	{		
		if(e.getActionCommand() == "EYS")		//스도쿠 시작하는 이벤트
		{
			Smethods.trysudoku(MySudoku.sudoku, (byte) 0);
			 MySudoku.step = 45;  //need to fill in 56 steps
		}
		...
		else if(e.getActionCommand() == "HINT")			//힌트를 누를 경우 하나를 오픈해준다
		{
			if(MySudoku.step < 81)
			MySudoku.step += 1;
		}
