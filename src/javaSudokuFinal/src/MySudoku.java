...
{
	public static byte[][] sudoku = new byte[729][82];  //global array for sudoku solution  // 수도쿠 솔루션 전역 배열
	public static byte step = 0; //global variable for solution step						// 솔루션 스텝 전연 벼누
	
	private static final int WindowWidth = 777; //its 777 pixels wide						// 폭 픽셀
	private static final int WindowHeight = 636; //its 636 pixels high 						// 높이 픽셀
	
    public static void ShowGUI() 
	 {
    	    Smethods.start(sudoku); //start array at step 0 has no numbers selected
    	   
		    final byte border = 14;  //border for display								// 게임 화면 경계
		    JFrame f = new JFrame("MySudoku");											// 프로그램 제목 이름
	        f.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);							// Close 를 누를경우 창을 닫음
	        BufferedImage image = null;													// 수도쿠 아이콘 이미지 변수 선언 
	            try {
					image = ImageIO.read(new File("sudoku.png"));						//sudoku.png파일을 아이콘 이미지로 넣어줌
				} catch (IOException e) {
				}//end of try/catch
	        f.setResizable(false);  // 게임 화면 사이즈 조정 불가
	        f.setIconImage(image);	// 아이콘 이미지 설정
		    f.setSize(WindowWidth, WindowHeight);  //size fixed by size of display and borders // 게임 화면 사이즈 설정
		    f.setLocation(0, 0); //start top left		// (0,0) 위치에서 화면을 띄움
	        ...
	        dp.setBackground(Color.BLACK);  //set the background of the sudoku display black // 배경을 검은색으로 설정
	        f.add(dp, BorderLayout.CENTER);  //add the sudoku display panel					// 게임 디스플레이 추가
	      
	        f.setVisible(true);				//창을 볼 수 있도록 설정
